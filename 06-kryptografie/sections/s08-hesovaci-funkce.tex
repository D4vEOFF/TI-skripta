...
\begin{definition}\label{def:krypto-hash}
    ...
    \]
    Pro $x\in\set{0,1}^{*}$ nazýváme hodnotu $H_n(x)$ \emph{hash} nebo \emph{otisk} řetězce $x$.
\end{definition}
...
\[
    \Pr_{x\in\set{0,1}^{n}}
    \left[
        H_n\bigl(\mathcal A(H_n(x))\bigr)=H_n(x)
    \right]
...
\subsection{Vztah k~jednosměrným funkcím}
