...
\subsection{Důsledek rovnosti tříd P a~NP}
...
\]
Jazyk $L_G$ patří do $\classNP$: jako certifikát stačí předložit seed $s$ a~v~polynomiálním čase ověřit rovnost $G(s)=z$. Kdyby platilo $\classP=\classNP$, mohli bychom členství v~$L_G$ rozhodovat v~polynomiálním čase.

Rozlišovač $\mathcal D$ by potom přijal právě řetězce z~$L_G$. Pro výstup $G(s)$ by přijímal vždy:
\[
    \Pr_{s\in\set{0,1}^{n}}[\mathcal D(G(s))=1]=1.
\]
Rovnoměrně náhodný řetězec délky $\ell(n)$ však patří do obrazu $G$ s~pravděpodobností nejvýše
\[
    \frac{2^n}{2^{\ell(n)}}=2^{n-\ell(n)}\leqslant\frac{1}{2}.
\]
...
\[
    ...
    \classP\neq\classNP.
\]
Opačná implikace z~pouhé nerovnosti $\classP\neq\classNP$ neplyne.
...
\subsection{Kandidáti a~padací vrátka}
