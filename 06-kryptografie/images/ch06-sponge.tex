\begin{tikzpicture}[
    ...
    \node[block] (m) at (-5.5,3) {zpráva \(m\)};
    \node[block] (pad) at (-2.8,3) {padding};
    \node[block, minimum width=3.2cm] (parts) at (.35,3)
        {bloky \(p_1,\ldots,p_t\)};
    \draw[flow] (m) -- (pad);
    \draw[flow] (pad) -- (parts);

    \node[state] (s0) at (-5.5,0) {\(S_0=0^b\)};
    \node[xor] (x1) at (-3.65,0) {\(\oplus\)};
    \node[func] (f1) at (-2.25,0) {\(f\)};
    \node[state] (s1) at (-.75,0) {\(S_1\)};
    \node at (.35,0) {\(\cdots\)};
    \node[xor] (xt) at (1.45,0) {\(\oplus\)};
    \node[func] (ft) at (2.85,0) {\(f\)};
    ...
    \node[block] (p1) at (-3.65,1.25) {\(p_1 0^c\)};
    \node[block] (pt) at (1.45,1.25) {\(p_t 0^c\)};
    \draw[flow] (parts.south west) to[bend right=8] (p1.north);
    \draw[flow] (parts.south east) to[bend left=8] (pt.north);
    \draw[flow] (p1) -- (x1);
    \draw[flow] (pt) -- (xt);

    \node[block, draw=green!55!black, fill=green!12] (rate) at (4.35,-1.7)
        {čti část rate};
    \node[state, minimum width=2.6cm] (hash) at (4.35,-3.15)
