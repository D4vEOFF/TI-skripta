...
\begin{figure}[ht]
    \centering
    \input{05-hw/images/ch05-pametova-hierarchie.tex}
...
\paragraph{Shrnutí kapitoly.}

\begin{itemize}
    \item Procesor vykonává instrukce, ale při své činnosti neustále komunikuje s~pamětí, vstupními a~výstupními zařízeními.
    \item Ve von Neumannově architektuře sdílejí instrukce a~data společnou paměť; harvardská architektura jejich paměti a~přenosové cesty odděluje.
    \item Základními částmi procesoru jsou výpočetní jednotky, řadič a~registry. Instrukce procházejí fázemi načtení, dekódování, získání operandů, provedení a~uložení výsledku.
    \item Cache zkracuje průměrnou dobu přístupu k~datům tím, že využívá časovou a~prostorovou lokalitu programů.
    \item Hlavní paměť poskytuje programu lineární adresový prostor, přestože jsou fyzické buňky v~čipech uspořádány do matic.
    \item HDD ukládá data magneticky na rotující plotny, zatímco SSD používá flash paměť bez pohyblivých částí.
    \item Registry, cache, RAM, úložiště a~archivní systémy společně vytvářejí paměťovou hierarchii, která vyvažuje rychlost, kapacitu a~cenu.
\end{itemize}
