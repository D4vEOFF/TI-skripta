...
\begin{example}
    ...
    %% TODO: Přidat výpočet pro konkrétní hodnoty proměnných
\end{example}
...
\begin{definition}[Polynomiální převoditelnost]\label{def:pol_prev}
    Řekneme, že libovolný problém $A$ je polynomiálně převoditelný na problém $B$, píšeme $A\rightarrow B$, pokud každý vstup $x$ problému $A$ lze převést na vstup $x^\prime$ problému $B$, přičemž $A(x)=B(x^\prime)$.
\end{definition}
