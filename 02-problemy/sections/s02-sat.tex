...
Nyní se zaměříme na samotnou splnitelnost vstupní formule. V~praxi se používají tzv. \emph{SAT solvery},~které rozhodují,~zda je formule v~CNF splnitelná. Ukážeme si klasický algoritmus \emph{DPLL},~jehož název připomíná příjmení Martina Davise,~Hilaryho Putnama,~George Logemanna a~Donalda Lovelanda. Jeho základní podoba pochází z~počátku 60.~let 20.~století.
...
\end{proof}
\notebox{Praktické SAT solvery formuli při každém volání celou nekopírují. Změny zapisují do pomocných struktur a~při návratu je vracejí zpět,~takže paměťový odhad lze podstatně zlepšit.}
