...
\begin{figure}[ht]
    \centering
    \input{01-grafalgo/images/ch01_ohod_graf.tex}
...
Ohodnocený graf budeme v~souladu s~prezentacemi zapisovat jako $G=(V,E,w)$,~kde $\mapping{w}{E}{\R}$ je váhová funkce. Je-li hrana určena koncovými vrcholy $u,v$,~píšeme pro stručnost $w(u,v)$.
...
\begin{figure}[ht]
    \centering
    \input{01-grafalgo/images/ch01_zapor_ohod_graf.tex}
...
\begin{figure}[ht]
    \centering
    \input{01-grafalgo/images/ch01_dijkstra_kratsi_cesta.tex}
...
Nejkratší vzdálenost mezi vrcholy $u$ a~$v$ označme $d(u,v)$. Důkaz vedeme indukcí podle pořadí uzavíraných vrcholů. Předpokládejme pro spor,~že $v$ je první uzavíraný vrchol,~pro který $D(v)>d(v_0,v)$. Vezměme jednu nejkratší cestu z~$v_0$ do $v$. Na této cestě musí existovat první dosud neuzavřený vrchol $x$; jeho předchůdce $y$ už uzavřený je. Podle indukčního předpokladu platilo při uzavření $y$ rovnost $D(y)=d(v_0,y)$. Algoritmus tehdy zkontroloval hranu $(y,x)$,~a~proto nastavil
\[D(x)\leqslant d(v_0,y)+w(y,x)=d(v_0,x)\leqslant d(v_0,v)<D(v).\]
...
\begin{figure}[ht]
    \centering
    \input{01-grafalgo/images/ch01_dijkstra_vyber_vrcholu.tex}
