...
Program~\ref{lst:dfs-python} zachovává rekurzivní podobu pseudokódu. Vnořená funkce \texttt{visit} odpovídá funkci \textsc{DFS2}: otevře zadaný vrchol, rekurzivně navštíví všechny jeho dosud nenalezené sousedy a~nakonec jej uzavře. Seznam \texttt{order} proto zaznamenává pořadí prvního otevření vrcholů, nikoli pořadí jejich uzavření.

Implementace navíc ukládá časy otevření a~uzavření a~předchůdce v~DFS stromu. Proměnná \texttt{time} je společná všem rekurzivním voláním, a~proto je ve funkci \texttt{visit} označena klíčovým slovem \texttt{nonlocal}. Program zpracuje pouze komponentu obsahující počáteční vrchol. Chceme-li vytvořit DFS les celého nesouvislého grafu, po návratu z~\texttt{visit(start)} zavoláme stejnou funkci také pro každý vrchol, který zůstal nenalezený.

\lstinputlisting[
    caption={Rekurzivní prohledávání grafu do hloubky v~Pythonu.},
    label={lst:dfs-python}
]{01-grafalgo/code/dfs.py}
